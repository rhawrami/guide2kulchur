"""
This script uses the user URLs indexed on the Goodreads author sitemap.
I used the script in 'scripts/supplements/sitmappin_userIDs.sh' to pull the IDs from there.
The resulting data is in a compressed file 'data/sitemap-dat/final_userIDs_from_sitemap.txt.gz', which I 
...
To be frank, this approach is very inefficient for pulling user records, as this sitemap is pretty outdated. The alternative
starting point for filling the user table is to pull random IDs in the higher range, with something like:

LOWER_END, UPPER_END = 1000000, 100000000
ids2pull = {random.randint(LOWER_END, UPPER_END) for _ in range(BATCH_SIZE)}

This would let us pull user records for users that (relatively) recently joined. But of course, we wouldn't 
get users who joined much earlier. No matter the process, we're going to get a lot of null/inactive/low-interaction users, which 
we'll drop from the DB based on some cutoff, maybe review_count or rating_count. In any case, this is a decent point to start, and later
we can try to pull through the random upper range ID approach listed above.
"""
...
def pull1ID_fromfile(f_path: str) -> Iterator[str]:
    '''yield one ID at a time from a file of user IDs'''
        ...
        with conn.cursor() as cur:
            async with aiohttp.ClientSession(timeout=timeout,
                ...
                if pull_the_ids:    # this way, we can skip this in case of a script error below this point
                    ...
                    filter_table_2newids = '''
                                            ...
                                            WHERE u_id = ANY(
                                                    ...
                                                    LEFT JOIN
                                                        false_dmitry dmtry
                                                    ON sm.u_id = dmtry.user_id
                                                    WHERE
                                                        dmtry.user_id IS NOT NULL
                                            )
                                        '''
                ...
                for batch_id in range(ITER_COUNT):
                    ...
                    perfectBlu = BatchUserPuller(batch_id=batch_id,
                                                 cursor=cur, 
                                                 user_ids=ids,
                                                 semaphore_count=sem_count,
                                                 status_logger=logger)
                    try:
                        await perfectBlu.load_the_batch(session=sesh,
                    ...
                    try:
                        perfectBlu.insert_failed_ids_into_db()  # in case of failed IDs, to ignore in the future
                        perfectBlu.insert_batch_into_db()
                    except Exception as er:
                    ...
                    sem_count, sub_batch_delay = update_sem_and_delay(current_sem_count=sem_count, 
                                                                      ...
                                                                      timeouts_per_batch_ratio=perfectBlu.metadat['timeouts_per_batch_ratio'],
                                                                      cfg=UPDATE_CFG)
                ...
                # drop_sitemapped_ids_table = '''DROP TABLE IF EXISTS sitemapped_ids_u'''
