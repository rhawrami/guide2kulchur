"""
This script will fill in the sim_authors column on our pound table.
"""

import asyncio
import os
import time

import aiohttp
import psycopg
from dotenv import load_dotenv
load_dotenv()

from guide2kulchur.engineer.simpullers import SimAuthorsPuller
from guide2kulchur.engineer.recruits import gen_logger, update_sem_and_delay


async def main():
    # update sim_authors in alexandria
    NAME = 'sim_authors'
    NAME_ABBR = 'sa'
    max_b = 5_000_000
    max_backups = 10

    logger = gen_logger(name=NAME, 
                        name_abbr=NAME_ABBR,
                        max_bytes_per_log=max_b,
                        max_backups=max_backups)
    
    pg_string = os.getenv('PG_STRING')

    # config timeout and connector
    timeout = aiohttp.ClientTimeout(total=12,
                                    connect=10)
    connector = aiohttp.TCPConnector(limit=30,
                                     limit_per_host=30,
                                     keepalive_timeout=120,
                                     enable_cleanup_closed=True)
    
    with psycopg.connect(conninfo=pg_string,
                         autocommit=True) as conn:
        with conn.cursor() as cur:
            async with aiohttp.ClientSession(timeout=timeout,
                                             connector=connector) as sesh:
                
                sem_count = 3   
                sub_batch_delay = 2  
                SUB_BATCH_SIZE = 10  
                NUM_ATTEMPTS = 3
                INTER_4BATCH_SLEEP = 10  

                NUM_ITER = 200  # this can always be changed, but for now, do 200 iterations
                BATCH_SIZE = (100,)    # 100 rows per batch, make tuple so we can pass to query

                CFG = {
                        'MIN_SEM': 3, 'MAX_SEM': 9,
                        'MIN_DELAY': .25, 'MAX_DELAY': 5,
                        'RATIO_THRESHOLD': .1, 'DELAY_DELTA': .5
                    }

                for batch_id in range(NUM_ITER):
                    if batch_id % 4 == 0:
                        time.sleep(INTER_4BATCH_SLEEP)  

                    logger.info('batch %s CFG: SEM-COUNT: %s & SUB-BATCH-DELAY: %s',
                                batch_id, sem_count, sub_batch_delay)

                    fetch_some_sim_ids_query = '''
                                                SELECT author_id
                                                FROM pound
                                                WHERE sim_authors IS NULL
                                                LIMIT %s
                                               '''
                    cur.execute(fetch_some_sim_ids_query, BATCH_SIZE)
                    sim_ids = {r[0] for r in cur.fetchall()}
                    
                    if not len(sim_ids):
                        logger.info('batch %s NO TUPLES LEFT', batch_id)    # means that all values in sim_authors col are filled :)
                        return None
                        
                    goethe = SimAuthorsPuller(batch_id=batch_id, 
                                              cursor=cur,
                                              sim_author_ids=sim_ids,
                                              semaphore_count=sem_count,
                                              status_logger=logger)
                    
                    try:
                        await goethe.load_the_batch(session=sesh,
                                                    batch_size=SUB_BATCH_SIZE,
                                                    batch_delay=sub_batch_delay,
                                                    num_attempts=NUM_ATTEMPTS)
                    except Exception as er:
                        logger.critical('ERR batch %s: %s', batch_id, er)   # not sure what would trigger this, but just in case
                        continue
                    
                    try:
                        goethe.insert_batch_into_db()
                    except Exception as er:
                        logger.critical('DB ERR batch %s: %s', batch_id, er)
                        continue
                    
                    sem_count, sub_batch_delay = update_sem_and_delay(current_sem_count=sem_count,
                                                                      current_sub_batch_delay=sub_batch_delay,
                                                                      timeouts_per_batch_ratio=goethe.metadat['timeouts_per_batch_ratio'],
                                                                      cfg=CFG)
            
            rm_sim_books_table_query = '''DROP TABLE IF EXISTS sim_authors'''
            cur.execute(rm_sim_books_table_query)                    
                    

if __name__ == '__main__':
    asyncio.run(main())

